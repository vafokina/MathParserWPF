...
namespace MathParserWPF.Model
{
    class MathInterpreter
    {
        ...
        private decimal ExecuteNode(AstNode node)
        {
            ...
            if (node.NodeType == AstNode.Type.Number)
            {
                ...
                bool ok = decimal.TryParse(node.Text, NumberStyles.Any, NFI, out decimal res);
                if (!ok) throw new ComputingException("Программа не может обработать такое большое или маленькое число.");
                return res;
            }
            decimal a = ExecuteNode(node.GetChild(0));
            decimal b = ExecuteNode(node.GetChild(1));
        ...
        public static decimal Execute(AstNode programNode)
        {
